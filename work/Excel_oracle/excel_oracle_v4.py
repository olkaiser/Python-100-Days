    ...
    if filename.endswith('.xlsx') and re.search(r'\d{4}年\d{2}月', filename):
        ...
        with pd.ExcelFile(excel_path) as xls:
            for sheet_name in xls.sheet_names:
                ...
                # header_list = df.iloc[header_row]
# print(header_list)

header_list=['asdfjkh','必选(sasdf0)','阿斯蒂芬啊(asdfjkh2)','阿斯蒂芬啊(asdfjkh3)','阿斯蒂芬啊(asdfjkh4)',
'阿斯蒂芬啊(asdfjkh5)','阿斯蒂芬啊(asdfjkh6)','我碟(next)','阿斯蒂芬啊(asdfjkh8)','阿斯蒂芬啊(asdfjkh9)','阿斯蒂芬啊(asdfjkh10)','asdf','阿斯蒂芬啊(asdfjkh12)']
...
for value in header_list:
    ...
    for col in df.columns:
        ...
        if value in df[col].values:
            # 找所有列的所有值
            value_index = df[col].values.tolist().index(value)

            # 找最大深度
            iloc_key = 0 
            if value_index > iloc_key:
                iloc_key = value_index
tmp_df = df.iloc[iloc_key+1:,:]
# print(iloc_key)
# print(df.iloc[iloc_key+1:,:])
def create_new_dataframe(DATAFRAM, header_list):
    new_dataframe = pd.DataFrame(DATAFRAM.values, columns=header_list)
    return new_dataframe
            # # list 要基于最大深度而不是+1
            # value_list = df[col].values.tolist()[value_index+1:]
new_df = create_new_dataframe(tmp_df,header_list)
print(new_df)
#             # print(value_list)
#             # print(str[value_list])
#             tmp_dick = {value:str(value_list)}
#             # print(tmp_dick)
#             tmp_dick.update(tmp_dick)
#         # print(tmp_dick)
#         #     break
#         new_data.update(tmp_dick)
#         # break
